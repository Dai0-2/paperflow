...
```text
为可选的 Google Drive 加密保险库安排有限的后台同步和失败重试任务。
```
...
- 用户活动：可选加密保险库中的阅读状态和最近阅读记录。
- 网络浏览记录：可选加密保险库中的论文来源网址和访问时间。
...
Google Drive 同步是可选功能，需要用户完成 Google 授权，并自行创建加密保险库密码。
```

不要向审核人员提供个人 Google 账号、API Key、保险库密码或恢复密钥。
