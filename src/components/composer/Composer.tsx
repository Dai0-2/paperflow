...
export function Composer() {
  ...
  return <div className="composer-zone">
    ...
    <div className="composer">
      <textarea ref={textarea} rows={1} value={value} onChange={(e) => setDraft(e.target.value)} placeholder={(providerMode === 'api' ? apiState : bridgeState) === 'connected' ? text(uiLanguage, 'Ask about this paper…', '向这篇论文提问…') : text(uiLanguage, 'Connect an AI provider to send…', '请先连接 AI 服务…')} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); void send(); } }} aria-label={text(uiLanguage, 'Ask about this paper', '向论文提问')} />
