...
export function SettingsView() {
  ...
  return <main className="view"><div className="view-header"><button className="back" onClick={() => setView('chat')}><ArrowLeft size={16} />{text(uiLanguage, 'Settings', '设置')}</button></div>
    <div className="view-content settings"><section><h3>{text(uiLanguage, 'AI provider', 'AI 服务')}</h3><div className="provider-list">
      <button data-active={providerMode === 'chatgpt'} onClick={() => setProviderMode('chatgpt')}><LogIn size={17} /><span><strong>{text(uiLanguage, 'ChatGPT subscription', 'ChatGPT 订阅')}</strong><small className={bridgeState === 'connected' ? 'connected' : ''}>{bridgeState === 'connected' ? text(uiLanguage, 'Connected with local Codex sign-in', '已使用本机 Codex 登录连接') : bridgeState === 'checking' ? text(uiLanguage, 'Checking…', '正在检查…') : bridgeDetail || text(uiLanguage, 'Uses your existing ChatGPT plan', '使用你现有的 ChatGPT 套餐')}</small></span>{providerMode === 'chatgpt' && <Check size={15} />}</button>
      {providerMode === 'chatgpt' && bridgeState !== 'connected' && <button className="provider-action" disabled={bridgeState === 'checking' || !extensionRuntimeAvailable} onClick={() => void connect()}>{!extensionRuntimeAvailable ? text(uiLanguage, 'Open the Chrome extension to sign in', '请在 Chrome 扩展中登录') : bridgeState === 'checking' ? text(uiLanguage, 'Opening sign-in…', '正在打开登录…') : text(uiLanguage, 'Sign in with ChatGPT', '登录 ChatGPT')}</button>}
      ...
      {providerMode === 'api' && (apiState === 'connected' ? <button className="provider-action danger" onClick={() => void removeKey()}><Trash2 size={14} />{text(uiLanguage, 'Remove API key', '删除 API 密钥')}</button> : <div className="api-key-row"><input type="password" autoComplete="off" spellCheck={false} value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder="API key" /><button disabled={apiState === 'checking' || !apiKey.trim() || !apiBaseUrl.trim() || !model.trim()} onClick={() => void saveKey()}>{text(uiLanguage, extensionRuntimeAvailable ? 'Save & test' : 'Test for this session', extensionRuntimeAvailable ? '保存并测试' : '仅本次测试')}</button></div>)}
