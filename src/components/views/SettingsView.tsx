import { ArrowLeft, BookOpen, Check, CloudUpload, Database, Globe2, KeyRound, Laptop, Languages, LogIn, Moon, Palette, RefreshCw, Server, Sun, Trash2, Type } from 'lucide-react';
...
const themes: { id: Theme; label: string; icon: typeof Sun }[] = [{ id: 'zotero', label: 'Zotero white', icon: Palette }, { id: 'light', label: 'Light', icon: Sun }, { id: 'dark', label: 'Dark', icon: Moon }, { id: 'system', label: 'System', icon: Laptop }];
...
export function SettingsView() {
  ...
  return <main className="view"><div className="view-header"><button className="back" onClick={() => setView('chat')}><ArrowLeft size={16} />{text(uiLanguage, 'Settings', '设置')}</button></div>
    <div className="view-content settings"><section><h3>{text(uiLanguage, 'AI provider', 'AI 服务')}</h3><div className="provider-list">
      ...
      {bridgeState !== 'connected' && <button className="provider-action" disabled={bridgeState === 'checking'} onClick={() => void connect()}>{bridgeState === 'checking' ? text(uiLanguage, 'Opening sign-in…', '正在打开登录…') : text(uiLanguage, 'Sign in with ChatGPT', '登录 ChatGPT')}</button>}
      <button data-active={providerMode === 'api'} onClick={() => setProviderMode('api')}><KeyRound size={17} /><span><strong>{text(uiLanguage, 'OpenAI-compatible API', 'OpenAI 兼容 API')}</strong><small className={apiState === 'connected' ? 'connected' : ''}>{apiState === 'connected' ? text(uiLanguage, 'Endpoint tested; key stored securely', '接口已测试；密钥已安全保存') : apiState === 'checking' ? text(uiLanguage, 'Checking…', '正在检查…') : apiDetail || text(uiLanguage, 'Official API or compatible relay', '官方接口或兼容中转站')}</small></span>{providerMode === 'api' && <Check size={15} />}</button>
      {apiState === 'connected' ? <button className="provider-action danger" onClick={() => void removeKey()}><Trash2 size={14} />{text(uiLanguage, 'Remove API key', '删除 API 密钥')}</button> : <div className="api-key-row"><input type="password" autoComplete="off" spellCheck={false} value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder="API key" /><button disabled={!apiKey.trim() || !apiBaseUrl.trim() || !model.trim()} onClick={() => void saveKey()}>{text(uiLanguage, 'Save & test', '保存并测试')}</button></div>}
      {providerMode === 'api' && <div className="endpoint-settings"><label><span><Server size={14} />Base URL</span><input value={apiBaseUrl} onChange={(event) => setApiBaseUrl(event.target.value)} placeholder="https://api.openai.com/v1" /></label><label><span>{text(uiLanguage, 'Model ID', '模型 ID')}</span><input value={model === 'ChatGPT via Codex' ? '' : model} onChange={(event) => setModel(event.target.value)} placeholder="gpt-4.1-mini" /></label><label><span>{text(uiLanguage, 'API format', '接口格式')}</span><select value={apiProtocol} onChange={(event) => setApiProtocol(event.target.value as typeof apiProtocol)}><option value="responses">Responses API</option><option value="chat-completions">Chat Completions</option></select></label><small>{text(uiLanguage, 'Enter a domain, /v1 base, or complete endpoint. HTTPS is required except for localhost.', '可填写域名、/v1 地址或完整接口地址；除 localhost 外必须使用 HTTPS。')}</small><button className="endpoint-test" disabled={apiState === 'checking' || !apiBaseUrl.trim() || !model.trim()} onClick={() => void testEndpoint()}><RefreshCw size={13} />{text(uiLanguage, 'Test connection', '测试连接')}</button></div>}
    </div></section>
      ...
      <section><h3>{text(uiLanguage, 'Appearance', '外观')}</h3><div className="theme-picker">{themes.map(({ id, label, icon: Icon }) => <button key={id} data-active={theme === id} onClick={() => setTheme(id)}><Icon size={15} />{id === 'zotero' ? text(uiLanguage, label, 'Zotero 白') : id === 'light' ? text(uiLanguage, label, '浅色') : id === 'dark' ? text(uiLanguage, label, '深色') : text(uiLanguage, label, '跟随系统')}</button>)}</div><div className="font-settings"><span><Type size={15} /><strong>{text(uiLanguage, 'Interface font', '界面字体')}</strong></span><div className="segmented three">{fontFamilies.map((option) => <button key={option.id} data-active={fontFamily === option.id} onClick={() => setFontFamily(option.id)}>{text(uiLanguage, option.en, option.zh)}</button>)}</div><label><span>{text(uiLanguage, 'Text size', '文字大小')}</span><input type="range" min="90" max="120" step="5" value={fontScale} onChange={(event) => setFontScale(Number(event.target.value))} /><output>{fontScale}%</output></label><small>{text(uiLanguage, 'Applies to PaperFlow controls and AI text, not the original PDF typography.', '应用于 PaperFlow 控件和 AI 文字，不改变 PDF 原始字体。')}</small></div></section>
