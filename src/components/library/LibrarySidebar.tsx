...
export function LibrarySidebar(props: LibrarySidebarProps) {
  ...
  return <aside className="library-sidebar">
    <nav className="library-primary-nav" aria-label="Library views">
      <ScopeButton icon={Library} label="All papers" count={saved.length} active={props.scope === 'all'} onClick={() => props.onScopeChange('all')} />
      <ScopeButton icon={Clock3} label="Recently added" active={props.scope === 'recent'} onClick={() => props.onScopeChange('recent')} />
      <ScopeButton icon={Star} label="Starred" count={saved.filter((paper) => paper.favorite).length} active={props.scope === 'favorite'} onClick={() => props.onScopeChange('favorite')} />
      <ScopeButton icon={BookOpen} label="Unread" count={saved.filter((paper) => paper.readStatus === 'unread').length} active={props.scope === 'status:unread'} onClick={() => props.onScopeChange('status:unread')} />
      <ScopeButton icon={BookOpen} label="Reading" count={saved.filter((paper) => paper.readStatus === 'reading').length} active={props.scope === 'status:reading'} onClick={() => props.onScopeChange('status:reading')} />
      <ScopeButton icon={BookOpen} label="Read" count={saved.filter((paper) => paper.readStatus === 'read').length} active={props.scope === 'status:read'} onClick={() => props.onScopeChange('status:read')} />
      <ScopeButton icon={Copy} label="Duplicates" active={props.scope === 'duplicates'} onClick={() => props.onScopeChange('duplicates')} />
      <ScopeButton icon={Trash2} label="Trash" count={trashed} active={props.scope === 'trash'} onClick={() => props.onScopeChange('trash')} />
    </nav>
    <CollectionTree
    ...
    <div className="tag-list">
      <div className="library-section-heading"><span>Tags</span><button title="Manage tags" onClick={props.onManageTags}><TagIcon /></button></div>
