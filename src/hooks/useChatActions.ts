...
import { cachePaperChunks, loadPaperChunks } from '../services/database';
import {
  buildPaperContext,
  chunksFromPages,
  citationsFromAnswer,
  pagesFromLegacyText,
} from '../services/paperContext';
...
export function useChatActions() {
  ...
  const submit = useCallback(async (question: string, replaceAssistantId?: string) => {
    ...
    if (replaceAssistantId) current.updateMessage(assistantId, {
      content: current.uiLanguage === 'zh' ? '正在重新生成…' : 'Regenerating…',
      model: current.model,
      ...
      current.addMessage({
        ...
        threadId: current.activeThreadId || undefined,
        role: 'assistant',
        content: current.uiLanguage === 'zh' ? '正在思考…' : 'Thinking…',
        model: current.model,
    ...
    const flushStream = () => {
      ...
      current.updateMessage(assistantId, {
        content: streamed,
        progress: current.uiLanguage === 'zh' ? '正在生成回答…' : 'Writing answer…',
        pending: true,
      });
    };
    try {
      let paperChunks = current.paperChunks;
      if (current.paper && !paperChunks.length) {
        paperChunks = await loadPaperChunks(current.paper.id).catch(() => []);
        if (paperChunks.length) current.setPaperChunks(paperChunks);
      }
      if (current.paper && !paperChunks.length && !current.selection) {
        current.updateMessage(assistantId, {
          content: '',
          progress: current.uiLanguage === 'zh' ? '正在首次建立论文索引…' : 'Building the paper index for the first time…',
          pending: true,
        });
        const paperText = await readPaperText(current.paper.url).catch(() => '');
        if (paperText) {
          paperChunks = await cachePaperChunks(
            current.paper.id,
            chunksFromPages(current.paper.id, pagesFromLegacyText(paperText)),
          );
          current.setPaperText(paperText);
          current.setPaperChunks(paperChunks);
        }
      }
      const history = current.messages
        .filter((message) => !message.pending && message.id !== replaceAssistantId)
        .slice(-6)
        .map((message) => `${message.role.toUpperCase()}: ${message.content}`)
        .join('\n\n')
        .slice(-8_000);
      const attachmentContext = current.attachments
        .filter((item) => item.kind !== 'image')
        .map((item) => `ATTACHMENT: ${item.name}\n${item.text}`)
        .join('\n\n')
        .slice(0, 16_000);
      const images = current.attachments
      ...
      const onEvent = (event: { event?: string; stage?: string; delta?: string }) => {
        if (event.event === 'delta' && event.delta) {
          ...
          if (streamTimer === undefined) streamTimer = setTimeout(flushStream, 32);
          return;
        }
        ...
        if (label) current.updateMessage(assistantId, {
          content: streamed,
          progress: current.uiLanguage === 'zh' ? label[1] : label[0],
        });
      };
      ...
      current.updateMessage(assistantId, result.ok && answer ? {
        content: answer,
        citations: citationsFromAnswer(answer, paperChunks),
        firstTokenMs: firstTokenMs ?? durationMs,
        durationMs,
        progress: undefined,
        pending: false,
      } : {
      ...
      current.updateMessage(assistantId, {
        content: reason instanceof Error
          ? reason.message
          : current.uiLanguage === 'zh' ? '无法获得回答。' : 'Unable to get a response.',
